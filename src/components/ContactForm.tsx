	...
	return (
		<form onSubmit={onSubmit} noValidate className="border border-line bg-white p-6 sm:p-8">
			...
			<div className="mt-6 grid gap-5 sm:grid-cols-2">
				<div>
					<label
						...
						className="font-mono text-[11px] uppercase tracking-wider text-slate"
					>
						{t('contact.form.name')}
					</label>
					<input
						...
						autoComplete="name"
						className="mt-2 w-full border border-line bg-paper px-3 py-2.5 text-sm text-ink placeholder:text-slate/60 focus:border-blue focus:outline-none"
					/>
					{errors.name ? (
						<p className="mt-1 font-mono text-[11px] text-sun-deep">{errors.name}</p>
					) : null}
				</div>
			...
			<div className="mt-5">
				...
				/>
				{errors.email ? (
					<p className="mt-1 font-mono text-[11px] text-sun-deep">{errors.email}</p>
				) : null}
			</div>
			...
			<div className="mt-5">
				...
				/>
				{errors.message ? (
					<p className="mt-1 font-mono text-[11px] text-sun-deep">{errors.message}</p>
				) : null}
			</div>

			<button
				type="submit"
				className="mt-6 bg-sun px-6 py-3 text-sm font-semibold text-ink transition-colors hover:bg-volt"
			>
